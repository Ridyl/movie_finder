...
export default function TopTen() {
	...
	return (
		<>
			{top.map((movie, index) => (
				<Card key={index} className='drop-shadow-xl mb-4 pt-0'>
					<img
						src={movie[0].Poster}
						className='rounded-t-xl max-h-60 object-cover'
					/>
					<CardHeader>
						<CardTitle>
							{index + 1}: {movie[0].Title}
						</CardTitle>
						<CardDescription>{movie[0].Actors}</CardDescription>
					</CardHeader>
					<CardContent className='line-clamp-2'>{movie[0].Plot}</CardContent>
					<CardFooter className='flex'>
						<div className='flex'>
							{movie[0].Ratings.map((rate, index) => (
								<div key={index} className='flex max-h-7'>
									{index === 0 ? (
						...
						<div>
							<Button className='ml-auto drop-shadow-sm' variant='outline'>
								More &#11167;
							</Button>
						</div>
					</CardFooter>
